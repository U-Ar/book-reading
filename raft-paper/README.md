...
以上によってLeader Completenessが保証され、それによって
...
サーバーがログエントリを状態機械に適用するとき、そのエントリまでのログがリーダーのものと完全に一致しており、それらのエントリはすべてコミットされている。

したがって、Leader Completenessより、以降のtermで選出されるリーダーは全て同一のログエントリを保持するため、以降のtermでそのインデックスのログエントリを状態機械に適用するサーバーにおいても全く同じ値が適用される。◽️

---

State Machine Safetyとログの適用順がインデックス順に限定されていることを合わせると、各サーバーの状態機械に適用される命令列が等しいことが保証される。

### 5.5 Follower and candidate crashes

RaftのRPCは冪等なので、フォロワーや候補者がRPCを受け取った後にクラッシュして復帰し、もう一度同じRPCを受け取っても問題ない。したがって、フォロワーや候補者が障害を起こした場合は単にリトライするだけでシンプルに対処できる。

### 5.6 Timing and availability

Raftはいつリクエストが来ても安全だが、可用性もそうとは限らない。例えば選挙中のリーダー不在時にリクエストが来ても応答できない。

リーダーを安定して稼動させるため、選挙タイムアウトはRPCブロードキャストにかかる時間よりも十分に長く、かつMTBF(Mean Time Between Failure)よりも十分に短い必要がある。

broadcastTime << electionTimeout << MTBF

これを踏まえて、選挙タイムアウトはだいたい10msから500msぐらいの値にするのが一般的


## 6 Cluster membership changes

ナイーブにクラスタ構成を変更しようとすると、クラスタを一度停止する必要がある。本章では、停止を挟まず、稼働中のクラスタの構成変更ができるような仕組みをRaftに組み込むことを考える。

すぐ思いつくのは古い設定から新しい設定に瞬間的に切り替える方法だが、これではある時点で1つのtermに複数のリーダーが選出される可能性がある(スプリットブレイン)。

安全性を維持しながら変更するためには2つのフェーズが必要になる。

1. joint consensus状態への移行
2. 新しいクラスタ構成への移行

joint consensusはクラスタ構成を移行するための中間状態で、以下のように動作する。

- ログエントリは新旧両方のクラスタの全サーバーに複製される
- 新旧どちらのクラスタ内のサーバーもリーダーになることができる
- 選挙やエントリのコミットは新旧クラスタそれぞれの過半数の合意が必要になる

具体的な移行手順:

- リーダーがクラスタ構成を $C_{old}$ から $C_{new}$ に変更するようリクエストを受け取ったとき、リーダーはまずjoint consensus状態 $C_{old,new}$ のログエントリを記録し、これを新旧全サーバーに複製する。このログエントリがコミットされたかどうかに関わらず、記録した時点でリーダーは $C_{old,new}$ のクラスタ構成を使い始める。(つまり、$C_{old,new}$ のログエントリがコミットされたかどうかの判断に $C_{old,new}$ のルールを使う。)
- 一度 $C_{old,new}$ がコミットされたら、 $C_{old}$ も $C_{new}$ ももう一方の合意なしでは決定を下せず、 $C_{old,new}$ を持つサーバーしかリーダーになれない。したがって、この時点でリーダーは $C_{new}$ を記述するログエントリを作成してクラスタ全体に複製できるようになる。 $C_{new}$ が $C_{new}$ のクラスタ構成のもとでコミットされたと判断できたら、もはや古いクラスタ構成は有効ではないので不要になったサーバーはシャットダウンできる。
...
### 問題1: 新しいサーバーは過去のログエントリを持たない

ログの最新化に時間がかかり可用性を損なう恐れがある。

対策としては、新しく加えたいサーバーをまず投票を行わないオブザーバーとしてクラスタに加え、最新化が完了した時点で構成変更を始めるのがよい

### 問題2: リーダーが新しいクラスタ構成に含まれない可能性がある

この場合、古いクラスタに属しているリーダーは $C_{new}$ のログエントリをコミットした時点で直ちにフォロワーに降格する。

$C_{new}$ がコミットされるまで、一時的にリーダーが「ログ複製は行なっているが自分自身をサーバーとしてカウントしない」という特殊な状態が発生する。

### 問題3: 古いサーバーがクラスタを妨害する可能性がある

移行完了後、古いクラスタ構成のサーバーはハートビートを受け取らなくなるので、選挙を開始する。このとき、 $C_{old}$ に属するサーバーたちにRequestVoteを送信して現在のリーダーを降格させてしまう可能性がある。新しくリーダーが選出された後も結局古いサーバーはハートビートを受け取らないのでまた選挙を開始し、以下繰り返し。

これを防ぐため、サーバーは現在のリーダーからのハートビートを受け取ってから選挙タイムアウトの最小時間よりも短い間隔でRequestVoteを受け取った場合、そのリクエストを無視するようにする。これによって通常の選挙には影響を与えず古いサーバーからの妨害を無視することができる。


## 7 Log compaction

ログが長くなってくると、メモリを圧迫する上最新化にかかる時間も長くなる。実用的にはどこかでログを圧縮する必要がある。

圧縮のためのアプローチは2種類

- スナップショット: ある時点での状態を永続化ストレージに保存し、それ以前のログを捨てる。ChubbyとかZooKeeperのやり方。
- インクリメンタルなアプローチ: log cleaning とか LSM tree とかのやり方。データの永続化と圧縮を時間的に分散しながら徐々に行う。log cleaningはRaftに変更を加えないと実現できないが、LSM treeはRaftのスナップショットと自然に統合できる。

### Raft式スナップショット

ログがフォロワーの状態機械に適用された時点で、そのログに関する合意が既に形成できている。したがって、フォロワーは現在のリーダーの情報に関わらず独立に自由なタイミングでローカルのスナップショットを取ることができる。

> このおかげで毎回リーダーからフォロワーへスナップショットを送る必要がなく、帯域も節約できるしリーダーの実装もシンプルになる

ただし、フォロワーが新しく参加したサーバーなど著しく遅れている場合はリーダーからスナップショットを送信することができる。そのための追加RPCとしてInstallSnapShot RPCを導入する。

スナップショットは以下の情報から構成する。

- その時点の状態機械の状態
- last included index: スナップショットに含まれる最後のログインデックス
- last included term: スナップショットに含まれる最後のログエントリのterm
- (クラスタ構成変更をしたい場合) スナップショットに含まれる最後のログ時点でのクラスタ構成


### パフォーマンスに関する問題

スナップショットを取る頻度が多すぎてもディスクを圧迫するし、少なすぎても再開時のログ最新化に時間がかかりすぎる。シンプルな戦略としてはログが一定の長さを超えたらスナップショットを取るようにすればよい。

また、スナップショットを取る操作は時間がかかるため、クライアントからのリクエストに遅延が発生する可能性がある。これを防ぐためにはスナップショットをcopy-on-writeにすればよい。

## 8 Client interaction

クラスタとクライアントのやり取りに関して。

- クライアントはまず適当なサーバーにリクエストを送る。
- リーダーだけがリクエストを直接処理し、それ以外のサーバーはリーダーが存在する場合はリーダーへリダイレクト、存在しない場合は単にリクエストを拒否する。拒否されたクライアントは単にリトライすればよい。

Read-only なリクエストはログに記録しないで処理するように最適化することも可能。ただし、古いデータを返さずlinearizabilityを維持するために2つの考慮が必要

1. リーダーはどのエントリがコミットされたかの最新情報を保持する必要がある。
2. リーダーはread-onlyリクエストを処理する前に自身がリーダーであることを確かめる必要がある。そのためにクラスタ全体にハートビートを送信する。ハートビートを受け取ってから選挙タイムアウトが経過するまではLeaseを取得したとみなし、Leaseを保持している間はハートビートなしで直接応答することもできるが、これは時刻の正確さに依存するので注意。

## 9 Implementation and evaluation

- やっぱりPaxosよりも理解しやすいと思うよという話
- Log Completenessのより形式的な証明をTLA+で書いたよという話
- State Machine Safetyについても非形式的だがより完全な証明を書いたよという話
- パフォーマンスは他の合意アルゴリズムと似たようなもの。最適化の余地はある
- 選挙タイムアウトの長さとランダム性はどれぐらいがいいか評価してみると、だいたい150ms-300msぐらいがおすすめ

## 10 Related Work

PaxosとRaftの主要な違いはRaftがリーダーに全面依存していること。でも、Paxosはパフォーマンスを最適化しようとすると結局アルゴリズムの正当性とは関係ないリーダー選出が必要になってきて複雑さが増す。その点、最初からリーダーをアルゴリズムに直接組み込んでいるRaftの方がシンプルに済む。

VRやZooKeeperはリーダー選出の仕組みを持っているという点でRaftと同様だが、フォロワーからリーダーへのデータの流れがある点でRaftよりも複雑。

また、メッセージの種類数が少ない点もRaftの強み。

- Raft: 4種類(RequestVote,AppendEntriesとそのレスポンス)
- ZooKeeper: 10種類
- VR: 10種類

クラスタの構成変更に関しても、クライアントのリクエストを処理しながら変更できるのがRaftの強み。

- VRは構成変更の間通常リクエストを全て止める
- SMARTはリクエストの数に制限をかける
