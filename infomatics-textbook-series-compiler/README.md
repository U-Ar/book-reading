...
5. $r$ が正規表現のとき、 $r^*$ は正規表現であり、 $L(r^*)=L(r)^*$ (L(r)の閉包)である
...
一般にプログラミング言語は三つの要素で定義される

1. 構文(syntax)：プログラムの構造を規定する
2. 制約(constraint)：構文的に正しいプログラムを実行するために要求される制約
3. 意味(semantics)：プログラムの構成要素がどのように実行されるか

これらをまとめて文法と称する。だが構文解析の文脈では1だけを指して文法ということも多い。本書もそう

本書では文脈自由文法というクラスに属する文法を取り扱う

> プログラミング言語ではそれが一般的だからとはいえ恣意的ということに注意

文脈自由文法：ある非終端記号Aが文形式に現れていれば、その前後の記号に関係なくAを左辺とする生成規則を適用できる文法。

> まさに規則の適用に関してcontext-free
