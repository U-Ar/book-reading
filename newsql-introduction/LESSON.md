...
### TSO(TimeStamp Oracle)の構成方法

内部的にはint64型の整数で、物理時間+論理時間で構成される

- 物理時間: ミリ秒単位のUnix時間
- 論理時間: 1ミリ秒内に複数のTSOを生成するためのカウンターで最大値は2^18

1ミリ秒あたり262144個のTSOが生成可能で実運用上被りはない

TSOはRaftによって選出されたPDのリーダーノードが生成する。生成したTSOはetcdに格納するため、リーダーが入れ替わっても過去のTSOに基づいて単調増加を維持する
